"""
This is a modified version of gameserver.py, which can be used for testing stuff outside the checker scripts.
The checker script is magically wrapped into a package , hence it needs relative imports like "import .cryptostuff".
These imports do not work locally, where we are not within the context of a proper python package.
"""

import socket
import json
from binascii import unhexlify, hexlify
# from cryptostuff import encrypt as crypto_encrypt
# import cryptostuff
...
class SimpleSocket(object):
    ...
    def sendline(self, msg: str):
        ...
        # l = self.recvline()
        # if l.strip() != msg.strip():
        #     raise MumbleException("Communication error")
...
class GameServer():
    ...
    @staticmethod
    def encryptAndStore_DONT_USE_ME_I_PRODUCTIVE_SCRIPTS(host: str, port: int, flagId: str, msg: str, method: str, timeout=DEFAULT_TIMEOUT):
        """
    ...
    def checkFlag(host: str, port: int, flagId: str, msg: str, timeout=DEFAULT_TIMEOUT):
        ...
        return currentParamHash != origParamHash
...
class GameServerCrypto:
    ...
    def parameterHash(parameters):
        ...
        m = str(
            parameters) + "42+4711+1337 blablubb!+klammerzu(die sha-klammer)und.digest()fehltauchnochundesmusseinhexdigestseindamitdiezeichenauchprintablesind!"
    ...
    def schwenkNew(msg: str, skipKeyAndIV=False):
        ...
        fakeHash = "".join(
            random.choice("0123456789abcdef") for _ in range(SHA512_HEX_LEN))  # To be consistent with the other plaintext formats
